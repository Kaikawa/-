...
% Written by Kenneth Train, Dec 11, 2007, revised for use with nest.m on Jan 6, 2008.

function ok=check

global NCS NROWS XMAT
global IDV NAMES B PREDICT
global NALT NEST1 LAMBDA
...
if ceil(NROWS) ~= NALT*NCS ;
   disp(['NROWS must equal NALT*NCS, but it is set to ' num2str(NROWS)]);
   disp('Program terminated.');
   return
end
...
if size(NEST1,1) ~= 1;
   disp(['NEST1 must have 1 row and yet it is set to have ' num2str(size(NEST1,1))]);
   disp('Program terminated.');
   return
end;

if size(NEST1,2) == 0;
   disp('NEST1 must have contain at least one alternative and yet it contains none.');
   disp('Program terminated.');
   return
end;

if sum(NEST1 > NALT,2) ~= 0;
   disp('NEST1 identifies an alternative that greater than NALT.');
   disp('NEST1 is');
   NEST1
   disp('when each element must be no greater than NALT=')
   disp([num2str(NALT)]);
   disp('Program terminated.');
   return
end;
...
if size(LAMBDA,1) ~= 1 | size(LAMBDA,2) ~= 1;
   disp(['LAMBDA must contain one element and yet it is set to have ' num2str(size(LAMBDA,1))]);
   disp('Program terminated.');
   return
end;
